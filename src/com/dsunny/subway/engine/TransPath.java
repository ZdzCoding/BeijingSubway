...
public class TransPath {
	...
	private List<String[]> getAllTransLids(List<String> lstStartSids, List<String> lstEndSids) {
		...
		switch (lstCommonLids.size()) {
			...
			case 1:
				...
				if (sCmnLid.equals(SubwayConst.Line_99)) {
					String startSid = mSDao
					        .getLineSIDById(SubwayConst.Line_99, lstStartSids.get(0));
		...
		for (String[] arr : lstResult) {
			Logger.d(TAG, arr == null ? "" : arr.toString());
		}
		return lstResult;
	}
	...
	private void addTransPathDetail(SearchResult sr, TransPathDetail tpd) {
		...
		if (!isExist) {
			...
			sr.price = sr.price < tpd.price ? sr.price : tpd.price;
		}
	}
